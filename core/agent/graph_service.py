...
def procesar_orden_grafo(orden: str, mem: dict, modo_autonomo: bool = True) -> str:
    ...
    if not final_response:
        import logging
        logging.getLogger("graph_service").warning(
            "final_response vacío tras invocar el grafo (orden=%r). "
            "Esto indica un bug upstream (ver _TOOLS_CON_RESUMEN / node_finalize): "
            "usando fallback genérico.",
            orden[:80],
        )
        final_response = "Operación completada."
