...
ERROR HANDLER (nuevo):
  Cubre los tres casos de fallo con diagnóstico web + confirmación del usuario
  + reintentos sin límite:

  1. Launch  — programa no encontrado o falla al lanzar
  2. Shell   — comando ejecutado que retorna error
  3. Código  — script generado que falla al ejecutar

  Flujo de error:
    fallo detectado
      → nodo_error_diagnose  (busca en web, LLM genera fix)
      → nodo_error_confirm   (muestra diagnóstico + diff al usuario, pide confirmación)
      → [usuario cancela → END]
      → nodo_error_retry     (aplica el fix y ejecuta de nuevo)
      → [sigue fallando → nodo_error_diagnose, sin límite de intentos]
      → [si web no ayuda → nodo_error_fallback, estrategias locales]
...
# Reutilizamos la lógica de detección de intención existente
...
# Reutilizamos la lógica de detección de intención existente
def nodo_router(state: AetherState):
    return state  # La lógica de routing ahora vive en core/services/aether_service.py
# ══════════════════════════════════════════════════════════════════════
# LLM SINGLETONS
# ══════════════════════════════════════════════════════════════════════
...
def _confirmar_usuario(mensaje: str) -> bool:
    """Pide confirmación al usuario. Siempre requerida en el error handler."""
    print(f"\n{mensaje}")
...
def nodo_lanzar_app(state: AetherState) -> AetherState:
    ...
    if app_id:
        ...
        state["error_contexto"]  = "launch"
        state["error_mensaje"]   = salida
        state["error_cmd"]       = cmd
        state["error_codigo_original"] = ""
        state["error_archivo"]   = ""
    ...
    state["error_contexto"]  = "launch"
    state["error_mensaje"]   = f"No se encontró ningún ejecutable para: {orden}"
    state["error_cmd"]       = ""
    state["error_codigo_original"] = ""
    state["error_archivo"]   = ""
    state["error_intento"]   = 0
...
def nodo_shell(state: AetherState) -> AetherState:
    ...
    if hubo_error:
        ...
        state["error_contexto"]        = "shell"
        state["error_mensaje"]         = salida
        state["error_cmd"]             = comando
        state["error_codigo_original"] = ""
        state["error_archivo"]         = ""
        state["error_intento"]         = 0
        # NO marcamos terminado — va al error handler
        return state
...
def nodo_escribir_archivo(state: AetherState) -> AetherState:
    ...
    # El usuario pidió probar el script → ejecutar y activar error handler si falla
    cmd_prueba = (
    ...
    if hubo_error:
        ...
        # Leer el código que escribimos para poder hacer diff luego
        try:
            with open(nombre_arch, "r", encoding="utf-8") as f:
                codigo_original = f.read()
        except Exception:
            codigo_original = respuesta  # fallback al texto del LLM

        state["error_contexto"]        = "codigo"
        state["error_mensaje"]         = salida_prueba
        state["error_cmd"]             = cmd_prueba
        state["error_codigo_original"] = codigo_original
        state["error_archivo"]         = nombre_arch
        state["error_intento"]         = 0
        state["salida_shell"]          = salida_prueba
        state["hubo_error"]            = True
        # NO terminamos — va al error handler
        return state
...
def nodo_error_confirm(state: AetherState) -> AetherState:
    ...
    if not confirmado:
        ...
        state["respuesta"] = (
            f"Operación cancelada tras {intento} intento(s). "
...
# Aplica el fix y ejecuta de nuevo
# Si vuelve a fallar → reactiva diagnóstico (sin límite de intentos)
# ══════════════════════════════════════════════════════════════════════
...
def nodo_error_retry(state: AetherState) -> AetherState:
    ...
    if hubo_error:
        print(f"\n❌ [RETRY]: Sigue fallando. Volviendo a diagnosticar (intento #{state.get('error_intento', 1) + 1})...")
        state["error_mensaje"] = salida
        state["error_cmd"]     = cmd
